...
<div class="container">
    ...
    <form action="/works/{{ $posts->id }}" method="POST">
        ...
        <h2>The Challenge</h2>
        <textarea name="challenge" rows="10">{{ $posts->challenge }}</textarea><br>
        <h2>Our Approach</h2>
        <textarea name="approach" rows="10">{{ $posts->approach }}</textarea><br>
        <h2>The Result</h2>
        <textarea name="result" rows="10">{{ $posts->result }}</textarea><br>
