...
use App\Exceptions\ValidationViewException;
...
class BaseModel extends Model
{
	 ...
	 * @param array $value [description]
	 */
	public function scopeSetStatusMessage($query, $value = [])
	{
	...
	public static function validate($data, $rules = [], $messages = [])
	{
		...
		if ($validator->fails()) throw new ValidationViewException($validator->errors());
		return true;
	}
	 ...
	 * @return [type]           [description]
	 */
	public static function validateAPI($data, $rules = [], $messages = [])
	{
		$rules = empty($rules) ? self::$rules : $rules;
		if (empty($rules)) return true;
		$validator = Validator::make($data, $rules, $messages);
		...
		if ($this->soft_delete) {
			\DB::table($this->table)
				...
				->update([
					...
					'delete_by' 	=> 	auth()->guard('contact')->user()->id, //user()->id,
